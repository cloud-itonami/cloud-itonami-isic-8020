(ns secsys.operation
  "OperationActor -- one security-systems-service coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (SecuritySystems-LLM) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Security Systems Governor (:govern) and the rollout phase gate
  ...
  decision to a human operator (security-systems operations
  coordinator). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:schedule-installation-operation`/
  `:flag-security-concern`/`:coordinate-equipment-supply` ALWAYS reach
  this node when the governor is clean -- see `secsys.phase`."
...
(defn build
  ...
  `secsys.store/Store`).
  opts:
    :advisor      -- a `secsys.secsysllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (secsysllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SecuritySystems-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (secsysllm/-advise advisor store request)]
            {:proposal p :audit [(secsysllm/trace request p)]})))

      ;; Security Systems Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
